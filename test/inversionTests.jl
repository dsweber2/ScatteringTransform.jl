...
# heatmap(reconstruct[1,:,:]); savefig("tmp.pdf")

# heatmap(abs.(fft(reconstruct[1,:,:]))); savefig("tmp.pdf")
# heatmap(abs.(fft(X3[1,:,:]))); savefig("tmp.pdf")
# heatmap(reconstruct[1,:,:] .- X3[1,:,:]); savefig("tmp.pdf")
# heatmap(X3[1,:,:]); savefig("tmp.pdf")
# heatmap(reconstruct[1,:,:]); savefig("tmp.pdf")
shears.dualFrameWeights - sum(abs.(shears.shearlets).^2, dims=3)
norm(X3-reconstruct,2)
@testset "inversion tests" begin
    @test typeof(reconstruct) <: typeof(X3)
    @test norm(X3-reconstruct,2)<.01 # the error in the reconstruction should be less than .1%
end
using LinearAlgebra
