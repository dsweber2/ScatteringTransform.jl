...
m3=2
X3 = zeros(Float32, 50, 50, 10);
X3[13:37, 13:37, 1] .= Float32(1);
layer3 = layeredTransform(m3, size(X3)[1:2], typeBecomes=eltype(
    X3))
@time asdf = st(X3, layer3, absType());
...
function testSubsampling(X::Array{T},layer::layeredTransform,m::Int64) where T<:Real
    ...
    for i=1:m1+1
        @test size(tmpX)[1:2] == (tmpSizesx[i],tmpSizesy[i])
...
function testShattered(X::Array{T,N},layer::layeredTransform,m::Int64) where {T <:Real,N}
    ...
    @test [size(thing.data[i], 2) for i=1:m+1] == sizes(bsplineType(),
                                                        layer.subsampling,
                                                        size(X, 2))[1:3]
    @test [size(thing.output[i],1) for i=1:m+1] == [outputSizes[i][1] for
                                                    i=1:m+1]
    @test [size(thing.output[i], 2) for i=1:m+1] == [outputSizes[i][2] for
                                                   i=1:m+1]
    # check the number of scales
    @test [size(thing.data[i], 3) for i=1:m+1] == [prod([1; q][1:i]) for i=1:m+1]
    @test [size(thing.output[i], 3) for i=1:m+1] == [prod([1; q][1:i]) for i=1:m+1]
end
...
function testShattering(X, nonlinear, layer, m)
    @time outputFull = st(X,layer,nonlinear,thin=false)
