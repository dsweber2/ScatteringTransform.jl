...
getQ(layers, Xsize; product = true) = getQ(layers, Int.(sizes(bspline, layers.subsampling, Xsize[(end):end])), [-1 for i = 1:depth(layers)+1]; product = product)
...
function createFFTPlans(layers::stParallel{<:Any,N}, dataSizes;
    ...
    for i in 1:nworkers(), j in 1:depth(layers)+1
        @debug "i=$(i), j=$(j)"
...
function createRemoteFFTPlan(sizeOfArray, T, iscomplex, nPlans::Int)
    if iscomplex
        return plan_fft!(zeros(Complex{T}, sizeOfArray...), (1,), flags = FFTW.PATIENT)
    elseif nPlans == 1
        return plan_rfft(zeros(T, sizeOfArray...), (1,), flags = FFTW.PATIENT)
    elseif nPlans == 2
        return (plan_rfft(zeros(T, sizeOfArray...), (1,), flags = FFTW.PATIENT),
            plan_fft!(zeros(Complex{T}, sizeOfArray...), (1,), flags = FFTW.PATIENT))
    end
end
...
function computeAllWavelets(layers, N; outputSubsample = (-1, -1), totalScales = [-1 for i = 1:depth(layers)+1], percentage = 0.9)
